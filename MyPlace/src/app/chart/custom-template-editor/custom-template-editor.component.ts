...
export class CustomTemplateEditorComponent implements OnInit, OnChanges {
    algorithms = [
    ...
    constructor(private pagerService: PagerService, private modalService: NgbModal, private specSrv: SpecificationService) {
    ...
    ngOnInit() {
        ...
        // load specifications
        this.specSrv.getSpecifications()
            .subscribe(data => this.specifications = data);
        ...
        this.templates$
            ...
            .subscribe(data => {
                ...
                this.fillTemplatesData("CycleNumber");
                this.setPage(1);
            });
	}
    ...
    dngAfterViewInit(): void {
        ...
        this.valueAxisItems = [
            ...
            { id: "CoulombicInefficiency", itemName: "Coulombic Inefficiency", type: "cycle" },
    ...
    onCreateTemplate() {
        //const editor = this.modalService.open(CustomTemplateEditComponent, { windowClass: 'template-editor-modal' });
        //editor.componentInstance.plotParameters = this.plotParameters;
    }
    ...
    onSave() {
        if (this.plotTemplate.name === "") {
			this.warningMessage = "Template name can not be empty";
			return;
        }
        if (this.plotTemplate.xAxis.valueField === "") {
			this.warningMessage = "Template xAxis should be specified";
			return;
        }
        if (this.selectedAxisItems.length === 0 && this.plotTemplate.yAxis.length === 0) {
			this.warningMessage = "Template yAxis values should be specified";
			return;
		}
        let seriType: string = "";
        if (this.selectedAxisItems[0].itemName.indexOf('(spec)') >= 0) {
            seriType = "scatter";
        }
        ...
        //if (this.selectedAxisItems[0].itemName.indexOf('(spec)') < 0)
        {
            ...
            if (this.plotTemplate.useCycleData && this.selectedAxisItems.length == 1) {
                ...
                if (this.plotTemplate.xAxis.name.indexOf("ChargeCapacity") >= 0 ||
                    ...
                    this.plotTemplate.xAxis.name.indexOf("CoulombicInefficiency") >= 0 ||
        ...
        //else {
        //    this.plotTemplate.useCycleData = false;
            this.plotTemplate.useSpecification = this.selectedAxisItems.filter(item => item.itemName.indexOf('(spec)') < 0).length > 0;
        //}
    ...
    fillTemplatesData(xName): void {
        this.dropdownListTemplates = [];
        for (const template of this.userPlotTemplates) {
            if (!(template.xAxis && template.xAxis.numerator && template.xAxis.numerator.arg1)) {
                console.error(`Template ${template.id} (${template.name}) doesn't have an argument axis setting`);
                continue;
            }
    ...
    onChangeFontSize(e): void {
		this.checkPlotTeplateExists();
		if (this.plotParameters.fontSize != e.element[0].children[0].value)
			this.plotParameters.fontSize = e.element[0].children[0].value;
    }

    onChangePointSize(e): void {
        this.checkPlotTeplateExists();
		if (this.plotParameters.pointSize != e.element[0].children[0].value)
    ...
    onChangeFontName(e): void {
		this.checkPlotTeplateExists();
		if (this.plotParameters.fontFamilyName != e.target.value)
	...
	onChangeSimpleAxis(e): void {
        ...
        if (this.selectedArgumentItem === "None") {
            ...
            this.fillValueAxisItems(false);
        }
    }

    onChangeXAxis(e): void {
        this.fillTemplatesData(e.target.value);
    }
    ...
    onEditTemplate(template, tabsComponent): void {
        ...
        this.fillValueAxisItems(true);
        this.selectedAxisItems = [];

        for (const axis of template.yAxis) {

            let nn = this.valueAxisItems.find(t => t.id === axis.name);
            ...
            nn = this.specifications.find(t => t.id.toString() === axis.name);

            if (nn != null || nn != undefined) {

                this.selectedAxisItems.push({
                    id: axis.name,
                    itemName: this.specifications.find(t => t.id.toString() === axis.name).name + " (spec)"
                });
            }
    ...
    fillValueAxisItems(isEdit: boolean): void {
        this.dropdownListAxis = [];
        ...
        //
        // add specifications
        // alert(this.selectedArgumentItem);
        if (this.selectedArgumentItem == "CycleNumber") {
            for (const spec of this.specifications) {
                if (spec.target == 1) {
                    this.dropdownListAxis.push({
                        id: spec.id.toString(),
                        itemName: spec.name + " (spec)"
                    });
                }
            }
        }
        else if (this.selectedArgumentItem == "Time")
        {
            for (const spec of this.specifications) {
                if (spec.target == 2) {
                    this.dropdownListAxis.push({
